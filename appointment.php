		...
		<?php
            ...
            if(isset($_POST['submit_book_appointment_form']) && $_SERVER['REQUEST_METHOD'] === 'POST')
            {
                ...
                try
                {
                    ...
                    $stmt_appointment = $con->prepare("INSERT INTO appointments(date_created, client_id, employee_id, start_time, end_time_expected) 
                                                       VALUES(?, ?, ?, ?, ?)");
                    $stmt_appointment->execute(array(Date("Y-m-d H:i"), $client_id, $selected_employee, $start_time, $end_time));

                    // ✅ Get the correct appointment_id
                    $appointment_id = $con->lastInsertId();

                    // ✅ Ensure `$selected_services` is not empty before looping
                    if (!empty($selected_services)) {
                        ...
                        // Optional: Add a message if no services are selected
